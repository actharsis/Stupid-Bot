...
class Websocket:
    def __init__(
        ...
        chatgpt.socket_closed[socket_uid] = False
        chatgpt.response_chunks[socket_uid] = []
        chatgpt.response_raw_chunks[socket_uid] = []
    ...
    def on_message(self, ws, message):
        ...
        self.chatgpt.response_raw_chunks[self.socket_uid].append(response)

        decoded_body = b64decode(response["body"]).decode("utf-8")
        response["body"] = decoded_body
        self.chatgpt.response_chunks[self.socket_uid].append(response)
    ...
    def on_close(self, ws, *args, **kwargs):
        self.chatgpt.socket_closed[self.socket_uid] = True
...
class ChatGPT:
    def __init__(
        ...
        self.cookies = utils.get_cookies(cookie_path)
        self.session.cookies.update(self.cookies)

        self.headers = utils.get_request_headers_and_append_auth(self)
        ...
        self.__title_cache = {}
        self.response_chunks: dict = {}
        self.response_raw_chunks: dict = {}
        self.socket_closed: dict = {}
        self.trace = trace
        self.max_socket_connections = 15
    ...
    async def ask(self, prompt: str):
        ...
        for socket in range(self.max_socket_connections):
            socket_uid = str(uuid.uuid4())
            ws = Websocket(ws_payload, self, socket_uid, self.trace)
            t1 = thr(target=ws.run)
            t1.start()
            response_index = -1
            last_message_status = "unknown"
            while not self.socket_closed.get(socket_uid) or response_index < len(self.response_chunks[socket_uid]) - 1:
                if response_index < len(self.response_chunks[socket_uid]) - 1:
                    response_index += 1
                    try:
                        chunk = self.response_chunks[socket_uid][response_index]["body"]
                        to_dict = json.loads(chunk[5:])
                        last_message_status = to_dict["message"]["status"]
                        if "is_completion" in to_dict.keys():
                            ...
                            continue
                        yield to_dict
                    except json.decoder.JSONDecodeError:  # [DONE\n\n]
                        break
                else:
                    await asyncio.sleep(0.25)

            t1.join()
            # print("socket state", self.socket_closed.get(socket_uid),
            #       "response state", response_index, len(self.response_chunks[socket_uid]))
            # for ans in self.response_chunks[socket_uid]:
            #     print(ans)
            self.socket_closed.pop(socket_uid)

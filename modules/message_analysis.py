...
def time_to_str(time):
    return f"{int(time // 60)} h, {int(time % 60)} min"
...
class AnalysisModule:
    ...
    async def get_user_scores(self, ctx):
        try:
            ...
            df = df.groupby(["author_id"]).sum().sort_values(
